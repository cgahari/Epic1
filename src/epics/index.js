...
function fetchStockPriceEpic(action$){
    console.log("HELLO ..45");
    return action$
        .ofType(FETCH_STOCK_PRICE)
        .switchMap((action) => {
            console.log("HELLO ..6799");
            return ajax
                .getJSON(`https://api.iextrading.com/1.0/stock/${action.payload}/price`)
                .map(data => fetchStockPriceSuccess(data))
                .catch(error => Observable.of(fetchStockPriceFaillure(error.message)))
        })
...
export const rootEpic = combineEpics(fetchStockPriceEpic,fetchWhiskiesEpic);
